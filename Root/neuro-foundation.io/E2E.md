...
of public keys and signatures.
[^RFC5639]: Brainpool curves are described in [RFC 5639](https://tools.ietf.org/html/rfc5639). This includes binary representations
of public keys and signatures.
...
by the element that encapsulates the encrypted data. For each symmetric cipher element defined, there is a corresponding attribute with the
same name in the `e2e` element. Supported symmetric ciphers, should declare this support by setting the corresponding attribute to `true`.
If the symmetric cipher attributes are omitted in the `e2e` element, it is assumed that all symmetric ciphers are supported. Available 
symmetric cipher elements and attributes include:

| Algorithm              | Element/Attribute | Security Level |
|:-----------------------|:------------------|---------------:|
...
| `k`       | `xs:base64Binary` | Optional | If the public-key algorithm referenced does not support derivation of shared keys (such as RSA), the symmetric cipher shared secret is explicitly generated by the sender, encrypted using the public key of the recipient, and sent in this attribute. |
...
<presence>
    <show>chat</show>
    <e2e xmlns="urn:nfi:iot:e2e:1.0" aes="true" cha="true" acp="true">
