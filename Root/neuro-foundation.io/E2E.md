...
| `k`       | `xs:base64Binary` | Optional | If the public-key algorithm referenced does not support derivation of shared keys (such as RSA), or emits a cipher text required by the recipient (such as with ML-KEM key encapsulation) the cipher text is sent in this attribute. |
